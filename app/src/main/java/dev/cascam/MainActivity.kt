...
class MainActivity : AppCompatActivity() {
    ...
    private fun configureForm(configuration: BroadcastConfiguration) {
        cameraIds = capabilities.rearCameras.map { it.id }
        val cameraLabels = capabilities.rearCameras.map {
            "ID ${it.id} · ${it.minimumFocalLength?.let { focal -> "$focal mm" } ?: "focal desconhecida"}"
        }
    ...
    private fun showCapabilities() {
        ...
        val concurrent = if (capabilities.supportsConcurrentRearCameras) "sim" else "não anunciado"
        binding.cameraStatus.text = "Traseiras: ${capabilities.rearCameras.size} · Ultra-wide provável: $ultraWide · Par simultâneo: $concurrent"
    }
